...
bool search(node* root,int d){      //O(logn) (when tree is balanced)
    if(root==NULL){
        return false;
    }
    if(root->data==d){
    ...
    if(d<=root->data){        //use if else here, don't make call on both the branches otherwise it will increase TC to O(n) in every case
